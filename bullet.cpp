...
#define	BULLET_HEIGHT		(50.0f)			// 頂点サイズ

#define	BULLET_SPEED		(3.0f)			// 弾の移動スピード
...
static float tw=1.0f;
static float th=1.0f;
static float tx=0.0f;
static float ty=0.0f;

static char *g_TextureName[] =
{
	"data/TEXTURE/flame_parts1.png",
};
...
HRESULT InitBullet(void)
{
	...
	for (int nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
	{
		...
		g_Bullet[nCntBullet].countAnim = 0;
	...
	for (int i = 0; i < MAX_BULLET; i++)
	{
		if (g_Bullet[i].use)
		{
			...
			if ((g_Bullet[i].countAnim % ANIM_WAIT) == 0)
			{
			 ...
			 tx = (float)(g_Bullet[i].patternAnim % TEXTURE_PATTERN_DIVIDE_X) * tw;	// テクスチャの左上X座標
			 ty = (float)(g_Bullet[i].patternAnim / TEXTURE_PATTERN_DIVIDE_X) * th;	// テクスチャの左上Y座標
...
HRESULT MakeVertexBullet(void)
{
	...
	// 頂点座標の設定
	vertex[0].Position = { -fWidth / 2.0f, 60.0f,  fHeight / 2.0f };
	vertex[1].Position = {  fWidth / 2.0f, 60.0f,  fHeight / 2.0f };
	...
	vertex[1].TexCoord = { tw, ty };
	vertex[2].TexCoord = { tx, th };
	vertex[3].TexCoord = { tw, th };
