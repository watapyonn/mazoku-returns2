...
static void SetFillBuffer(void);
...
static ID3D11Buffer*			g_FillBuffer = NULL;	//hlslに渡す
...
//シェーダーの練習
static FIll_CBUFFER		g_Fill;			//これは軽油してg_FillBufferに渡す
											//FILL　　　　　　　→　FIll_CBUFFER　→　ID3D11Buffer*
											//レンダーヘッダー　→　このフィル　　→　このフィル
...
//void SetFill(int flag,XMFLOAT4 color)
//{
//	g_Fill.Color  = color;
//	g_Fill.Enable = flag;
//	//この下の関数でシェーダーに渡している	
//	GetDeviceContext()->UpdateSubresource(g_FillBuffer, 0, NULL, &g_Fill, 0, 0);
//}

////塗りつぶしよう（シェーダー練習）

void SetFillBuffer(void)
{
	GetDeviceContext()->UpdateSubresource(g_FillBuffer, 0, NULL, &g_Fill, 0, 0);
}
...
void SetFill(FILL* pFill)
{
	g_Fill.Color = pFill->Color;
	SetFillBuffer();
}
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	//塗りつぶし(シェーダー練習)
	hBufferDesc.ByteWidth = sizeof(FIll_CBUFFER);
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_FillBuffer);
	g_ImmediateContext->VSSetConstantBuffers(8, 1, &g_FillBuffer);
	g_ImmediateContext->PSSetConstantBuffers(8, 1, &g_FillBuffer);
