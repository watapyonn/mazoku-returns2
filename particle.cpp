...
#define	MAX_PARTICLE		(200)		// パーティクル最大数
...
HRESULT InitParticle(void)
{
	...
	for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		...
		g_aParticle[nCntParticle].time = 0.0f;			// 線形補間用のタイマーをクリ
		g_aParticle[nCntParticle].tblNo = 0;			// 再生するアニメデータの先頭アドレスをセット
		g_aParticle[nCntParticle].tblMax = sizeof(move_tbl) / sizeof(INTERPOLATION_DATA);	// 再生するアニメデータのレコード数をセット
